...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function loadErrors(page = 1) {
            ...
            fetch(`/api/logs/error?page=${page}`, { headers })
                .then(res => {
                ...
                .then(res => {
                    ...
                    if (!data || data.length === 0) {
                        tbody.innerHTML = `<tr><td colspan="8" class="text-center">Tidak ada data log error.</td></tr>`;
                        return;
                    }

                    data.forEach((log, index) => {
                        const shortAgent = log.agent ? log.agent.slice(0, 60) + '...' : '-';
                        ...
                        tbody.innerHTML += `
                            <tr>
                                ...
                                <td><span title="${log.agent}">${shortAgent}</span></td>
                        ...
                        cardContainer.innerHTML += `
                            <div class="card mb-3">
                                <div class="card-body">
                                    ...
                                    <div><strong>User Agent:</strong> ${shortAgent}</div>
                ...
                .catch(error => {
                    ...
                    document.getElementById('logErrorBody').innerHTML =
                        `<tr><td colspan="8" class="text-center text-danger">${error.message}</td></tr>`;
                });
        }
